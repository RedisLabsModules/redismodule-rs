...
impl Subscribers {
    ...
    fn get_or_create_subscribers_list(&mut self, ctx: &Context) -> Result<&mut Vec<ServerEventCallback>, RedisError>{
        if self.get_subscribers_list().is_none() {
            unsafe{
                if raw::RedisModule_SubscribeToServerEvent.unwrap()(
                    ...
                    return Err(RedisError::Str("Failed subscribing to server role changed event"));
                }
    ...
    fn subscribe_to_event(&mut self, ctx: &Context, callback: ServerEventCallback) -> Result<(), RedisError> {
...
extern "C" fn role_changed_callback(ctx: *mut raw::RedisModuleCtx, _eid: raw::RedisModuleEvent, subevent: u64, _data: *mut ::std::os::raw::c_void) {
    ...
    for callback in unsafe{&ROLE_CHANGED_SUBSCRIBERS}.get_subscribers() {
        callback(&ctx, ServerEventData::RoleChangedEvent(RoleChangedEventData{role: new_role.clone()}));
    }
}

fn subscribe_to_role_changed_event(ctx: &Context, callback: ServerEventCallback) -> Result<(), RedisError> {
...
extern "C" fn loading_event_callback(ctx: *mut raw::RedisModuleCtx, _eid: raw::RedisModuleEvent, subevent: u64, _data: *mut ::std::os::raw::c_void) {
    ...
    for callback in unsafe{&LOADING_SUBSCRIBERS}.get_subscribers() {
        callback(&ctx, ServerEventData::LoadingEvent(loading_sub_event.clone()));
    }
}

fn subscribe_to_loading_event(ctx: &Context, callback: ServerEventCallback) -> Result<(), RedisError> {
...
extern "C" fn flush_event_callback(ctx: *mut raw::RedisModuleCtx, _eid: raw::RedisModuleEvent, subevent: u64, _data: *mut ::std::os::raw::c_void) {
...
fn subscribe_to_flush_event(ctx: &Context, callback: ServerEventCallback) -> Result<(), RedisError> {
    unsafe{&mut FLUSH_SUBSCRIBERS}.subscribe_to_event(ctx, callback)
}

pub fn subscribe_to_server_event(ctx: &Context, event: ServerEvents, callback: ServerEventCallback) -> Result<(), RedisError> {
    match event {
