...
fn call_test(ctx: &Context, _: Vec<RedisString>) -> RedisResult {
    ...
    if "TEST" != &res? {
        return Err(RedisError::Str("Failed calling 'ECHO TEST' dynamic str vec"));
    }

    let res: Result<String, RedisError> = ctx.call("ECHO", &[b"TEST"])?.into();
    if "TEST" != &res? {
        return Err(RedisError::Str("Failed calling 'ECHO TEST' with static [u8]"));
    }

    let res: Result<String, RedisError> = ctx.call("ECHO",vec![b"TEST"].as_slice())?.into();
    if "TEST" != &res? {
        return Err(RedisError::Str("Failed calling 'ECHO TEST' dynamic &[u8] vec"));
    }
    ...
    let res: Result<String, RedisError> = ctx.call("ECHO",vec![&"TEST".to_string()].as_slice())?.into();
    if "TEST" != &res? {
        return Err(RedisError::Str("Failed calling 'ECHO TEST' dynamic &[u8] vec"));
    }
    ...
    if "TEST" != &res? {
        return Err(RedisError::Str("Failed calling 'ECHO TEST' with RedisString"));
    }

    let res: Result<String, RedisError> = ctx.call("ECHO", vec![&ctx.create_string("TEST")].as_slice())?.into();
    if "TEST" != &res? {
        return Err(RedisError::Str("Failed calling 'ECHO TEST' with dynamic array of RedisString"));
    }
